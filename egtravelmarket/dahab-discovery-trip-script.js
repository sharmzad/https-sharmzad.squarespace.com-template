...
async function loadDahabDates() {
  try {
    ...
    if (dateSelect) {
      ...
      const dahabDates = data.dahab_discovery_trip || [];
      dahabDates.forEach(d => {
        if (d.seats_left > 0) {
    ...
    console.error('Error loading Dahab dates:', err);
    const dateSelect = $('#date');
    if (dateSelect) {
      dateSelect.outerHTML = '<select id="date" required></select>';
      const newDateSelect = $('#date');
...
function updateDahabSummary() {
  ...
  addText('Tour: ', true);
  addText('Dahab Discovery Trip – Full Day Adventure');
  addBreak();
  ...
  addBreak();
  addText('⚠️ Important: Passport required', false, 'font-size:0.9rem;color:#666;');
  addBreak(); addBreak();
...
async function processDahabPayment(event) {
  const date = $('#date').value;
  const name = $('#name').value.trim();
  const email = $('#email').value.trim();
  const phone = $('#phone').value.trim();
  ...
  specialRequests += 'Duration: Full day (08:00 - 17:00)\n⚠️ Important: Passport required';
  
  var ids = resolveTripId(13);
  if (!validateTripIdBeforePayment(ids)) return;

  const bookingData = {
    tripId: ids.tripId,
    expertTripId: ids.expertTripId,
    packageName: "Dahab Discovery Trip",
    packageType: "dahab_discovery_trip",
    guestName: name,
    guestEmail: email,
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (adultsInput) adultsInput.addEventListener('change', () => { updateSubtotal(); updateDahabSummary(); });
  if (childrenInput) childrenInput.addEventListener('change', () => { updateSubtotal(); updateDahabSummary(); });
  if (infantsInput) infantsInput.addEventListener('change', updateDahabSummary);
  if (dateInput) dateInput.addEventListener('change', updateDahabSummary);
  if (nameInput) nameInput.addEventListener('input', updateDahabSummary);
  if (emailInput) emailInput.addEventListener('input', updateDahabSummary);
  if (phoneInput) phoneInput.addEventListener('input', updateDahabSummary);
  if (hotelInput) hotelInput.addEventListener('input', updateDahabSummary);
