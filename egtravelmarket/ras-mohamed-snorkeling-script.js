...
async function loadRasMohamedDates() {
  try {
    ...
    if (dateSelect) {
      ...
      const rasDates = data.ras_mohamed_snorkeling || [];
      rasDates.forEach(d => {
        if (d.seats_left > 0) {
    ...
    console.error('Error loading Ras Mohamed dates:', err);
    const dateSelect = $('#date');
    if (dateSelect) {
      dateSelect.outerHTML = '<select id="date" required></select>';
      const newDateSelect = $('#date');
...
function updateRasMohamedSummary() {
  ...
  addText('Tour: ', true);
  addText('Ras Mohamed Snorkeling Boat Trip');
  addBreak();
  ...
  payBtn.addEventListener('click', async (e) => {
    e.preventDefault();
    await processRasMohamedPayment(e);
  });
...
async function processRasMohamedPayment(event) {
  const date = $('#date').value;
  const name = $('#name').value.trim();
  const email = $('#email').value.trim();
  const phone = $('#phone').value.trim();
  ...
  specialRequests += 'Note: Park entry fee (5 USD/person) payable onboard';
  
  var ids = resolveTripId(8);
  if (!validateTripIdBeforePayment(ids)) return;

  const bookingData = {
    tripId: ids.tripId,
    expertTripId: ids.expertTripId,
    packageName: "Ras Mohamed Snorkeling",
    packageType: "ras_mohamed_snorkeling",
    guestName: name,
    guestEmail: email,
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (adultsInput) adultsInput.addEventListener('change', () => { updateSubtotal(); updateRasMohamedSummary(); });
  if (childrenInput) childrenInput.addEventListener('change', () => { updateSubtotal(); updateRasMohamedSummary(); });
  if (dateInput) dateInput.addEventListener('change', updateRasMohamedSummary);
  if (nameInput) nameInput.addEventListener('input', updateRasMohamedSummary);
  if (emailInput) emailInput.addEventListener('input', updateRasMohamedSummary);
  if (phoneInput) phoneInput.addEventListener('input', updateRasMohamedSummary);
  if (hotelInput) hotelInput.addEventListener('input', updateRasMohamedSummary);
