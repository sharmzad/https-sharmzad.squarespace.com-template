...
function updateSubtotal() {
  ...
  const priceAdult = 250;
  const priceChild = 240;
...
function updatePetraSummary() {
  ...
  addText('Tour: ', true);
  addText('Petra by Boat - Egypt to Jordan Day Trip');
  addBreak();
...
async function processPetraPayment(event) {
  const date = $('#date').value;
  const name = $('#name').value.trim();
  const email = $('#email').value.trim();
  const phone = $('#phone').value.trim();
  ...
  specialRequests += '⚠️ Passport copy required via WhatsApp 1 day before trip';
  
  var ids = resolveTripId(7);
  if (!validateTripIdBeforePayment(ids)) return;

  const bookingData = {
    tripId: ids.tripId,
    expertTripId: ids.expertTripId,
    packageName: "Petra by Boat",
    packageType: "petra_by_boat",
    guestName: name,
    guestEmail: email,
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (adultsInput) adultsInput.addEventListener('change', () => { updateSubtotal(); updatePetraSummary(); });
  if (childrenInput) childrenInput.addEventListener('change', () => { updateSubtotal(); updatePetraSummary(); });
  if (dateInput) dateInput.addEventListener('change', updatePetraSummary);
  if (nameInput) nameInput.addEventListener('input', updatePetraSummary);
  if (emailInput) emailInput.addEventListener('input', updatePetraSummary);
  if (phoneInput) phoneInput.addEventListener('input', updatePetraSummary);
  if (hotelInput) hotelInput.addEventListener('input', updatePetraSummary);
