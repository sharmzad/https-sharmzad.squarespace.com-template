...
function updateSubtotal() {
  ...
  const priceAdult = 270;
  const priceChild = 260;
...
function updateLuxorSummary() {
  ...
  addText('Tour: ', true);
  addText('Luxor Day Trip from Sharm El Sheikh by Flight');
  addBreak();
...
async function processLuxorPayment(event) {
  const date = $('#date').value;
  const name = $('#name').value.trim();
  const email = $('#email').value.trim();
  const phone = $('#phone').value.trim();
  ...
  var ids = resolveTripId(5);
  if (!validateTripIdBeforePayment(ids)) return;

  const bookingData = {
    tripId: ids.tripId,
    expertTripId: ids.expertTripId,
    packageName: "Luxor by Flight",
    packageType: "luxor_by_flight",
    guestName: name,
    guestEmail: email,
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (adultsInput) adultsInput.addEventListener('change', () => { updateSubtotal(); updateLuxorSummary(); });
  if (childrenInput) childrenInput.addEventListener('change', () => { updateSubtotal(); updateLuxorSummary(); });
  if (infantsInput) infantsInput.addEventListener('change', updateLuxorSummary);
  if (dateInput) dateInput.addEventListener('change', updateLuxorSummary);
  if (nameInput) nameInput.addEventListener('input', updateLuxorSummary);
  if (emailInput) emailInput.addEventListener('input', updateLuxorSummary);
  if (phoneInput) phoneInput.addEventListener('input', updateLuxorSummary);
  if (hotelInput) hotelInput.addEventListener('input', updateLuxorSummary);
