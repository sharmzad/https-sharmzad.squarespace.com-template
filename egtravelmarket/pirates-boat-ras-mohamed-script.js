...
async function loadPiratesDates() {
  try {
    ...
    if (dateSelect) {
      ...
      const piratesDates = data.pirates_boat_ras_mohamed || [];
      piratesDates.forEach(d => {
        if (d.seats_left > 0) {
    ...
    console.error('Error loading Pirates Boat dates:', err);
    const dateSelect = $('#date');
    if (dateSelect) {
      dateSelect.outerHTML = '<select id="date" required></select>';
      const newDateSelect = $('#date');
...
function updatePiratesSummary() {
  ...
  addText('Tour: ', true);
  addText('Pirates Boat to Ras Mohamed - Full Day Snorkeling Adventure');
  addBreak();
  ...
  addBreak();
  addText('Note: Park entry fee (5 USD/person) not included.', false, 'font-size:0.9rem;color:#666;');
  addBreak(); addBreak();
...
async function processPiratesPayment(event) {
  const date = $('#date').value;
  const name = $('#name').value.trim();
  const email = $('#email').value.trim();
  const phone = $('#phone').value.trim();
  ...
  specialRequests += 'Duration: Full day (08:00-17:00)\nNote: Park entry fee (5 USD/person) payable onboard';
  
  var ids = resolveTripId(6);
  if (!validateTripIdBeforePayment(ids)) return;

  const bookingData = {
    tripId: ids.tripId,
    expertTripId: ids.expertTripId,
    packageName: "Pirates Boat – Ras Mohamed",
    packageType: "pirates_boat_ras_mohamed",
    guestName: name,
    guestEmail: email,
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (adultsInput) adultsInput.addEventListener('change', () => { updateSubtotal(); updatePiratesSummary(); });
  if (childrenInput) childrenInput.addEventListener('change', () => { updateSubtotal(); updatePiratesSummary(); });
  if (infantsInput) infantsInput.addEventListener('change', updatePiratesSummary);
  if (dateInput) dateInput.addEventListener('change', updatePiratesSummary);
  if (nameInput) nameInput.addEventListener('input', updatePiratesSummary);
  if (emailInput) emailInput.addEventListener('input', updatePiratesSummary);
  if (phoneInput) phoneInput.addEventListener('input', updatePiratesSummary);
  if (hotelInput) hotelInput.addEventListener('input', updatePiratesSummary);
